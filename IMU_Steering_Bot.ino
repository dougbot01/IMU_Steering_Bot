...
// Variables added for steering control
...
void loop() //Main Loop
{
  //First check for command messages from Serial port
  if (Serial.available() ) {
    message.process(Serial.read());
  }
  
  // Update heading from IMU at 50Hz
  if((millis()-timer)>=20)
  {
    counter++;
    timer_old = timer;
    ...
    //if (counter==0) printdata();
  }
  
  // Update calculations for steering
  steeringCalc();
